...
def simulate_single_trace(p_prot=0.7, k_prot=200, p_dna=0, k_dna=None, position_blur=0.1, force_blur=1, l_dna=350,
    ...
    real_rupture_forces = list(np.array(rupture_forces) + np.random.normal(0, force_blur, len(l_prots)-1))
    real_rupture_forces.append(force_range[-1])
    if p_dna > 0:
        rupture_dna_distances = [l_dna * invert_wlc(f, p_dna, k_dna) for f in real_rupture_forces]
    else:
        rupture_dna_distances = list(np.zeros(len(real_rupture_forces)))
    d_prot_rupture = [l_prots[_] * invert_wlc(real_rupture_forces[_], p_prot, k_prot) for _ in range(len(l_prots) - 1)]
